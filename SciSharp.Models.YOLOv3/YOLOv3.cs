...
namespace TensorFlowNET.Examples.ImageProcessing.YOLO
{
    public class YOLOv3
    {
        ...
        public Tensor Decode(Tensor conv_output, int i = 0)
        {
            ...
            conv_output = tf.reshape(conv_output, new[] { batch_size, output_size, output_size, tf.constant(3), tf.constant(5 + num_class) });
            ...
            var y = tf.tile(range[Slice.All, tf.newaxis], new[] { tf.constant(1), output_size });
            var x = tf.tile(range[tf.newaxis, Slice.All], new[] { output_size, tf.constant(1) });

            var xy_grid = tf.concat(new[] { x[Slice.All, Slice.All, tf.newaxis], y[Slice.All, Slice.All, tf.newaxis] }, axis: -1);
            xy_grid = tf.tile(xy_grid[tf.newaxis, Slice.All, Slice.All, tf.newaxis, Slice.All], new[] { batch_size, tf.constant(1), tf.constant(1), tf.constant(anchor_per_scale), tf.constant(1) });
        ...
        public (Tensor, Tensor, Tensor) loss_layer(Tensor conv, Tensor pred, Tensor label, Tensor bboxes, NDArray anchors, int stride)
        {
            ...
            var bbox_loss_scale = 2.0f - 1.0f *
                label_xywh[":", ":", ":", ":", "2:3"] *
                label_xywh[":", ":", ":", ":", "3:4"] / tf.pow(input_size, 2);
            var giou_loss = respond_bbox * bbox_loss_scale * (1.0f - giou);

            var iou = bbox_iou(pred_xywh[Slice.All, Slice.All, Slice.All, Slice.All, tf.newaxis, Slice.All], 
                bboxes[Slice.All, tf.newaxis, tf.newaxis, tf.newaxis, Slice.All, Slice.All]);
            var max_iou = tf.expand_dims(tf.reduce_max(iou, axis: -1), axis: -1);

            var respond_bgd = (1.0f - respond_bbox) * tf.cast(max_iou < iou_loss_thresh, tf.float32);
